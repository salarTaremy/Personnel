...
{
    public partial class FrmViewDailyMission : Permission.ClsMyForms
    {
        ClsDailyMission DM = new ClsDailyMission();
        ClsPersonel Person = new ClsPersonel();
        public FrmViewDailyMission()
        {
            InitializeComponent();
        }

        private void FrmViewDailyMission_Load(object sender, EventArgs e)
        {
        ...
        private void FillGrid()
        {
            try
            {
                 this.Grd.DataSource = DM.GetAll(Person,this.UcDatePicker1.TxtInt,this.UcDatePicker2.TxtInt);
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                FrmDailyMission Frm = new FrmDailyMission();
                Frm.ShowDialog();
            }
            catch (Exception Ex )
            {
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                FrmDailyMission Frm = new FrmDailyMission((long)this.Grd.CurrentRow.Cells["ID"].Value);
                Frm.ShowDialog();
                FillGrid();
            }
            catch (Exception ex)
        ...
        private void BtnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string Msg = string.Format("آیا برای حذف ماموریت مورخه'{0}' تا '{1}'  مربوط به '{2}' مطمئن هستید ؟", (string)this.Grd.CurrentRow.Cells["FromDate"].Value, (string)this.Grd.CurrentRow.Cells["Todate"].Value,  (string)this.Grd.CurrentRow.Cells["fullName"].Value);
                ...
                if (DM.Delete(ID) >0)
	            {
		        Global.Dialog.ShowOK("حذف ماموریت با موفقیت انجام شد")    ;
                FillGrid();
	            }
                else
                {
        ...
        private void BtnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                int RowAffect = DM.SetConfirm(true, Grd.GetSelectedItems("ID"));
                if (RowAffect > 0)
                {
        ...
        private void BtnDeConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                int RowAffect = DM.SetConfirm(false, Grd.GetSelectedItems("ID"));
                if (RowAffect > 0)
                {

    ...
    public partial class FrmViewHourlyMission : Permission.ClsMyForms
    {
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                FrmHourlyMission Frm = new FrmHourlyMission((long)this.Grd.CurrentRow.Cells["ID"].Value);
                Frm.ShowDialog();
                FillGrid();
            }
            catch (Exception ex)
        ...
        private void FrmViewMission_Load(object sender, EventArgs e)
        {
        ...
        private void FillGrid()
        {
            try
            {
                this.Grd.DataSource = Ms.GetAllMission(Person, this.UcDatePicker1.TxtInt, this.UcDatePicker2.TxtInt);
                ...
                this.Grd.Columns["Type"].HeaderText = "نوع ماموریت";
        ...
        {
            FrmHourlyMission Frm = new FrmHourlyMission();
            Frm.ShowDialog();
            FillGrid();
        }
        ...
        private void BtnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Dialog.ShowInfo("عملیات حذف ماموریت ها فقط بصورت تک ایتمی امکان پذیر میباشد" + Environment.NewLine + "لطفا فقط یک گزینه را انتخاب نمایید");
                    return;
                }
                string Msg = string.Format("آیا برای حذف ماموریت مورخه{0} مربوط به {1} مطمئن هستید ؟" + Environment.NewLine + "از ساعت '{2}' تا '{3}'", (string)this.Grd.CurrentRow.Cells["Date"].Value, (string)this.Grd.CurrentRow.Cells["fullName"].Value, this.Grd.CurrentRow.Cells["FromTime"].Value.ToString(), this.Grd.CurrentRow.Cells["ToTime"].Value.ToString());
                ...
                if (Ms.Delete(ID) >0)
	            {
		        Dialog.ShowOK("حذف ماموریت با موفقیت انجام شد")    ;
                FillGrid();
	            }
                else
                {
        ...
        private void BtnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                int RowAffect =  Ms.SetConfirm(true, Grd.GetSelectedItems("ID"));
                if (RowAffect > 0)
                {
            ...
            {
                Dialog.ShowErorr("متاسفانه عملیات تایید با خطا مواجه شد", ex.Message);
            }
        }
        ...
        private void BtnDeConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                int RowAffect = Ms.SetConfirm(false, Grd.GetSelectedItems("ID"));
                if (RowAffect > 0)
                {

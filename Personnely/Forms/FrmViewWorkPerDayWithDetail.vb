...
Public Class FrmViewWorkPerDayWithDetail
    ...
    Private Sub FillGrid()
        Try
            If TxtPrID.Value > 0 AndAlso Pr.Exists = False Then
                Dialog.ShowInfo("لطفا کد پرسنلی معتبری را وارد کنید")
                Exit Sub
            End If
            Me.Grd.DataSource = pw.ViewWorkPerDay(UcDatePicker1.TxtInt, UcDatePicker2.TxtInt, Pr.CodePersoneli, ChkViewLapse.Checked, ChkViewMission.Checked, ChkViewLeave.Checked)
            With Me.Grd
                If .Columns.Contains("PrCode") Then .Columns("PrCode").HeaderText = "کد پرسنلی"
                ...
                If .Columns.Contains("IO") Then .Columns("IO").IsVisible = False
                ...
                .Columns("DayOfWeek").Width = 240
                .Columns("DayName").Width = 100
                .Columns("Date").Width = 70
                .Columns("ActionType").Width = 80
                .Columns("ActionName").Width = 95
                .Columns("time").Width = 55
                .Columns("Action").Width = 210
                .Columns("HolidayName").Width = 310
                .Columns("Counter").Width = 220
                .Columns("IsLapse").Width = 230
                .Columns("IsManual").Width = 240
            End With
        ...
        Try
            For i As Integer = 0 To Me.Grd.Rows.Count - 1
                With Me.Grd
                    ...
                    End If
                    If CBool(.Rows(i).Cells("isLapse").Value) = True AndAlso .Rows(i).Cells("Counter").Value IsNot DBNull.Value Then
                    ...
                    End If
                    If .Rows(i).Cells("Counter").Value Is DBNull.Value Then
                        If .Rows(i).Cells("IO").Value.ToString = "M" Then
                            Me.Grd.SetRowForeColor(i, Color.Orange)
                            Grd.Rows(i).Cells("HolidayName").Value &= " ماموریت "
                        ElseIf .Rows(i).Cells("IO").Value.ToString = "L" Then
                            Me.Grd.SetRowForeColor(i, Color.Green)
                            Grd.Rows(i).Cells("HolidayName").Value &= " مرخصی "
                        End If
                    End If
                End With
            Next
